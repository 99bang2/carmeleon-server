...
exports.pgPaymentNice = async function (ctx) {
	...
	if (payInfo.totalPrice) {
		...
		let buyerName 		= ctx.user.nickname || '사용자'
	...
	if(paymentFlag) {// success
		...
		if (payInfo.coopPayment) {
			await common.updateCoopPayment(ctx, coopPaymentData)
		}
		//포인트 사용
		if(payInfo.point > 0){
...
exports.pgPaymentCancelNice = async function (ctx) {
	...
	// 	}
	// 	await payInfo.update({status: -20, cancelStatus: 10, cancelReason: reason, cancelCompleteTime: Sequelize.fn('NOW')})
